...
def unpack_json(game_json):
    ...
    for country_name in game['countries']:
        country = board['countries'][country_name]
        game_country = game['countries'][country_name]
        ...
        for enemy_country in country[
            ...
            if board['countries'][enemy_country]['owner'] != my_name:
                country['bordering_enemy_troops'] += (
                     countries[enemy_country]['troops'])

        country['unique_bordering_enemies'] = list(set(
                        country['bordering_enemies']))

        for unique_enemy in (
             country['unique_bordering_enemies']):
        ...
        country['strategic_value'] = (
                set_strategic_value(board, country_name, me))

        country['threat_value'] = (
    ...
    enemy_troops_per_turn = sum(
        [board['other_players'][enemy]['troops_per_turn'] for enemy in
         board['countries'][country_name]['unique_bordering_enemies']
                                                    if enemy != 'none'])
    enemy_card_worth = 0
    sets_exchanged = (44-board['cards_left'] -
                      sum([board['other_players'][player]
...
def set_strategic_value(board, country_name, me):
    country = board['countries'][country_name]
    continent = board['continents'][board['countries']
                                    [country_name]['continent']]

    num_bordering_countries = len(country['bordering_countries'])
    num_bordering_enemies = len(country['bordering_enemies'])
    access_points = continent['access points']
...
def choose_country(board):
    unoccupied = [c for c in board['countries']
                  if board['countries'][c]['owner'] == 'none']
...
def attack_determination(board, me):
    ...
    for attack_combo in possible_attacks:
        if (board['countries'][attack_combo[0]]['troops']
                >= 2 * board['countries'][attack_combo[1]]['troops']):
...
def attack(board, possible_attacks):
        ...
        a = board['countries'][attacking_country]
        d = board['countries'][defending_country]
        for attack in possible_attacks:
            if board['countries'][attack[1]]['strategic_value'] > max_value:
                ...
                if (board['countries'][attack[0]]['troops'] >
                        a['troops']):
                    attacking_country = attack[0]
        attacking_troops = min(3, a['troops'] - 1)
        moving_troops = 0
        troops_available = max(0, a['troops'] - 4)
        threat = d['threat_value']
        ...
        print "attacking %s from %s with %s troops" % (defending_country,
                                                       attacking_country,
                                                       attacking_troops)
...
def reinforce(board, me):
    ...
    if reinforce_from is not None:
        ...
        response = {'action': 'reinforce',
                    'data': {'origin_country': reinforce_from,
